...
<head>
  ...
  <title>HUỶ PHIẾU YÊU CẦU - Anova Farm</title>
...
<html>
  ...
  <body>
    ...
    <div class="container my-4">
      ...
      <center><p class="h3">HUỶ PHIẾU YÊU CẦU</p></center>
        ...
        <div class="card-body">
          ...
          <form action="" method="post">
            ...
            <?php
                ...
                $sql_man_approval = "SELECT * FROM staff WHERE `email` LIKE '$email';";
            ...
            <div class="clearfix mt-4" style="text-align: center;">
              <?php if($levelid==3||$levelid==2){ if($approval==1 AND $id_client == $user_request){?>
              <!--<button  type="submit" name="submit" value="3" class="btn btn-primary float-center text-uppercase shadow-sm" >ĐỒNG Ý</button>-->
              <button style="margin-left: 30px;" type="submit" name="submit" value="4" class="btn btn-primary float-center text-uppercase shadow-sm" >XÁC NHẬN HUỶ</button>
              <?php }else if($approval==4||$approval==6 ){?>
              <button style="margin-left: 30px;background-color:  lightgray;color:black" class="btn btn-primary float-center text-uppercase shadow-sm" disabled>ĐÃ HUỶ TOA THUỐC</button>
              <?php }}?>
            </div>
          </form>
        </div>
...
<?php 
            ...
            /*foreach ($run_pre_details as $row_pre_details) {
              ...
              $id_staff = $row_staff['id'];
                if($unit_check=="Chai"||$unit_check=="Lọ"){
                  $sql_debt = "INSERT INTO `debt` (`id`, `id_farm`, `id_staff`, `id_med`, `amount_box`, `date_start`, `date_end`, `status`, `date_create`) VALUES (NULL, '$farm_details', '$user_request', '$id_med', '$amount_box', CURRENT_TIMESTAMP, NULL, '1', CURRENT_TIMESTAMP);";
                  $run_debt = $con->query($sql_debt);
                }
              }*/


            $sql_emailad = "SELECT * FROM staff WHERE status =1 AND `id_farm` LIKE '$farm' AND admin_farm =1";
            ...
            $subject = '[ĐỒNG Ý] Toa Thuốc của  '.$row_emailad['fullname'];
            ...
            //DENY CONDITIONAL
                ...
                //RUN UPDATE CANCEL PRESCRIPTION
                $sql_update = "UPDATE `prescription` SET `approval` = '4' , `date_approval`= CURRENT_TIMESTAMP WHERE `prescription`.`id` = $id_pheduyet;";
                $run_update = $con->query($sql_update);

                
                //Cập Nhập Dữ Liệu Tồn Kho khi PYC tự huỷ
            ...
            //Nếu Tự Huỷ PYC Sẽ Gửi Mail cho Người Tạo email
            ...
            $subjectdeny = '[HUỶ TOA] Toa Thuốc của  '.$row_emailad['fullname'];
            //Nội Dung Gửi mail cho User Tạo Toa Thuốc
            $message = "Dear {$row_emailad['fullname']},";
            $message .= "<br/><br/>Toa thuốc đã được bạn huỷ thành công.";
            $message .= "<br/> <p>Chi Tiết Toa Thuốc Tại: <a href='http://traiheo.anovafarm.com.vn/admin/import/prescription/history/chitiet.php/?id={$id_pheduyet}'>XEM NGAY</a></p>";
            $message .= "<br/><img src='https://traiheo.anovafarm.com.vn/admin/mail/signature.png' alt='Signature Of Anova Farm' style='width:50%; text-align:left'>";
            // Set the sender and recipient email addresses
            $mail->setFrom($sender, "TOA THUỐC - ANOVAFARM");
