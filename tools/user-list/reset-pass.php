...
<html>
<head>
  <title>Reset Tài Khoản | Anova Farm</title>
...
//GET INFO USER
$sql_user ="SELECT * FROM user WHERE `id_staff`=$id_client";
...
$sql_staff = "SELECT * FROM staff WHERE `id`=$id_client";
...
<form method="POST" action="reset-pass.php?<?php echo http_build_query($_GET); ?>"> 
<style>
  table {
    width: 100%;
        ...
        <div class="card-body">
          <table>
            ...
            <tr>
              ...
              <td><input type="text" name="fullname" value="<?php echo $fullname_staff;?>" disabled>

            </tr>
            <tr>
              <td><label>User:</label></td>
              <td><input type="text" name="username_user" value="<?php echo $username; ?>" disabled>
            </tr>
...
if($_POST["new_password"]==""||$_POST["retype_new_password"]==""||$new_password!=$retype_new_password){
  echo "";
}else {
		$update = "UPDATE `user` SET `password` = MD5('$new_password') WHERE `user`.`id_staff` = $id_client;";
	}
