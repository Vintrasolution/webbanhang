            ...
            <?php
              ...
              if (!is_dir($targetDir)) {
                  if (!mkdir($targetDir, 0755, true)) {
                      die("Không thể tạo thư mục lưu trữ.");
                  }
              }
              ...
              if(isset($_FILES['image_upload'])){
                  ...
                  if($_FILES['image_upload']['error'] === UPLOAD_ERR_OK){
                      ...
                      $destPath = $targetDir . basename($newFileName);

                      // Di chuyển file từ thư mục tạm đến thư mục mục tiêu
                      if(move_uploaded_file($fileTmpPath, $destPath)){
                          echo "<strong>Hình ảnh sản phẩm:</strong> <br/> <img src='$destPath' width='400px'>";
                      } else{
                          echo "Có lỗi xảy ra khi upload file.";
                      }
                  } else{
                      // Xử lý các mã lỗi upload
                      switch($_FILES['image_upload']['error']){
                          case UPLOAD_ERR_INI_SIZE:
                              echo "Lỗi: Kích thước file vượt quá giới hạn cho phép trong php.ini.";
                              break;
                          case UPLOAD_ERR_FORM_SIZE:
                              echo "Lỗi: Kích thước file vượt quá giới hạn cho phép trong form.";
                              break;
                          case UPLOAD_ERR_PARTIAL:
                              echo "Lỗi: File chỉ được upload một phần.";
                              break;
                          case UPLOAD_ERR_NO_FILE:
                              echo "Lỗi: Không có file nào được upload.";
                              break;
                          case UPLOAD_ERR_NO_TMP_DIR:
                              echo "Lỗi: Thiếu thư mục tạm thời.";
                              break;
                          case UPLOAD_ERR_CANT_WRITE:
                              echo "Lỗi: Ghi file lên đĩa thất bại.";
                              break;
                          case UPLOAD_ERR_EXTENSION:
                              echo "Lỗi: Một tiện ích mở rộng PHP đã ngăn cản việc upload file.";
                              break;
                          default:
                              echo "Lỗi: Không xác định được lỗi upload.";
                              break;
                      }
                  }
              } else{
              ...
              ?>
              <input type="hidden" name="url_image" value="<?php echo $destPath; ?>">
              <input type="hidden" name="name_image" value="<?php echo $newFileName; ?>">
            <br/><br/>
            <!-- Thành tiền chưa VAT-->
...
<?php         
             if($_POST['submit']==4){
                  ...
                  //Thêm URL Image
                  $sql_url= "INSERT INTO `billing_image` (`id`, `id_billing`, `name_image`, `url`, `status`) VALUES (NULL, '$id_bill', '$name_image', '$url_image', '1');";
                  $run_url = $con->query($sql_url);
