      ...
      <div class="card my-4 shadow">
        <div class="card-body">
          ...
          <form action="" method="post">
            ...
            ?>
            <br/>
            <label for="field" class="font-weight-bold">Người Duyệt:  </label>
            <?php
                $email = $row_farm['email'];
                $sql_man_approval = "SELECT * FROM user as u , staff as s WHERE u.id_staff = s.id AND u.level = 4 AND s.id_farm LIKE '$farm' AND s.status =1 AND u.status=1";
            ...
            <div class="clearfix mt-4" style="text-align: center;">
              <?php if($levelid==2||$levelid==3){ if($approval==3 AND $id_client == $user_request){?>
              <button  type="submit" name="submit" value="7" class="btn btn-primary float-center text-uppercase shadow-sm" >GỬI YÊU CẦU</button>
              <button style="margin-left: 30px;" type="submit" name="submit" formaction="chitiet.php?id=<?php echo $id_toa; ?>" class="btn btn-primary float-center text-uppercase shadow-sm" >QUAY LẠI</button>
              <?php }else if($approval==5||$approval==6){?>
              ...
              <?php } $success = $_GET['success']; if($success==1){echo "<a style='color:green'>ĐÃ GỬI YÊU CẦU THÀNH CÔNG</a>";}}?>
            </div>
          </form>
        </div>
      </div>
...
<?php 
            ...
            //AGREE CONDITIONAL
            if($_POST['submit']==7){
            //CẬP NHẬP LẠI TRẠNG THÁI TOA THUỐC THÀNH ĐANG CHỜ DUYỆT
            $sql_update = "UPDATE `prescription` SET `approval` = '7' , `date_approval`= CURRENT_TIMESTAMP WHERE `prescription`.`id` = $id_pheduyet;";
            $run_update = $con->query($sql_update);

            //GỬI YÊU CẦU ĐẾN TRƯỞNG TRẠI
            //Email Trưởng Trại
            $sql_tt_mail = "SELECT * FROM user as u , staff as s WHERE u.id_staff = s.id AND u.level = 4 AND s.id_farm LIKE '$farm' AND s.status =1 AND u.status=1";
            $run_tt_mail = $con->query($sql_tt_mail);
            $row_tt_mail = mysqli_fetch_array($run_tt_mail);
            $to = $row_tt_mail['email'];
            $namett = $row_tt_mail['fullname'];

            //Email Phó Trại
            $sql_pt_mail = "SELECT * FROM user as u , staff as s WHERE u.id_staff = s.id AND u.level = 6 AND s.id_farm LIKE '$farm' AND s.status =1 AND u.status=1";
            $run_pt_mail = $con->query($sql_pt_mail);
            $row_pt_mail = mysqli_fetch_array($run_pt_mail);
            $to_pt = $row_pt_mail['email'];

            //Tiêu Đề Gửi mail cho Trưởng Trại
            $subject = '[YÊU CẦU HUỶ] Toa Thuốc của  '.$row_staff_request['fullname'];
            //Nội Dung Gửi mail cho Admin Trại
            $message = "Dear {$namett},";
            $message .= "<br/><br/>".$row_staff_request['fullname']." Gửi yêu cầu xin huỷ toa thuốc.";
            $message .= "<br/> <p>Chi Tiết Toa Thuốc Tại: <a href='http://traiheo.anovafarm.com.vn/admin/import/prescription/approval-cancel/?id={$id_pheduyet}'>XEM NGAY</a></p>";
            ...
            ?><meta http-equiv="refresh" content="0; url='?id=<?php echo $id_pheduyet;?>&success=1'" /><?php
