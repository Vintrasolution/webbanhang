...
<?php 
            ...
            $sql_emailad = "SELECT * FROM staff WHERE status =1 AND `id_farm` LIKE '$id_trai' AND admin_farm =1";
            $run_emailad = $con->query($sql_emailad);
            $row_emailad = mysqli_fetch_array($run_emailad);
            $email_admin = $row_emailad['email'];
            //$to = $email;
            //TÌM MAIL THỦ KHO
            $sql_thukho = "SELECT * FROM staff as s , user as u WHERE u.level = 5 and s.id = u.id_staff and s.id_farm LIKE '$id_trai';";
