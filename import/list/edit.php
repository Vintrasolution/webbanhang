      ...
      <div class="card my-4 shadow">
        <div class="card-body">
          ...
          <?php 
            ...
            $sql_supplier = "SELECT * FROM import_details WHERE status =1 AND id = '$id_suppliers'";
            ...
            //Thông Tin Bill Trước chỉnh sửa
              ...
              //Get Name Pay Method follow ID
              $sql_methodpayment = "SELECT * FROM payment_method_import where id = '$id_pay_method_import'";
              $run_methodpayment = $con->query($sql_methodpayment);
              $row_methodpayment = mysqli_fetch_array($run_methodpayment);
              $name_use = $row_methodpayment['name_method'];
          ?> 
          
          <!-- Thông tin trại --> 
          <form onsubmit="return validateForm()" action="" method="post" name="myForm">
            <!-- ID Import Details -->
            <input type="text" name="id_suppliers" class="form-control" value="<?php echo $id_suppliers; ?>" hidden>
            <!-- Tạo Ngày Nhập Hàng -->
            ...
            <p>
              ...
              <select class="js-example-basic-multiple form-control js-example-placeholder-multiple" id="field" name="id_supplier" >
                  ...
                  <?php 
                    $sql_supplier = "SELECT * FROM `supplier` WHERE status=1 and id !='$id_supplier'";
                    $run_supplier = $con->query($sql_supplier);
                  ?>
            ...
            <p><input type="date" name="date_import" class="form-control" value="<?php echo $date_import; ?>"></p>

            <label for="field" class="font-weight-bold">Loại Hàng Hoá: <a style="color:red">*</a></label>
            <p><textarea rows="5" cols="50" name="details" class="form-control" placeholder="Nhập chi tiết hàng hoá nhập vào"><?php echo $product_type; ?></textarea></p>

            <label for="field" class="font-weight-bold">Đơn Giá: <a style="color:red">*</a></label>
            <p><input type="number" name="price_unit" id="price_unit" class="form-control" placeholder="VNĐ" oninput="tinhThanhTien()" value="<?php echo $price_unit; ?>"></p>
            <label for="field" class="font-weight-bold">Số Lượng: <a style="color:red">*</a></label>
            <p><input type="number" name="quantity" id="quantity" class="form-control" placeholder="Số Lượng" oninput="tinhThanhTien()" value="<?php echo $quantity; ?>"></p>
            <label for="field" class="font-weight-bold">Thành Tiền: <a style="color:red">*</a></label>
            <p><input type="text" name="price_total" id="price_total" class="form-control" readonly placeholder="Tổng Tiền" value="<?php echo $price_total; ?>"></p>
            <label for="field" class="font-weight-bold">Hình Thức Thanh Toán: <a style="color:red">*</a></label>
            <p>
              <select name="pay_method" class="form-control">
                <option value="<?php echo $id_pay_method_import;?>"><?php echo $name_use; ?></option>
                <?php 
                  $sql_paymethod = "SELECT * FROM `payment_method` WHERE status=1 and id != $id_pay_method_import";
            ...
            <div class="clearfix mt-4">
              <button value="1" class="btn btn-primary float-left text-uppercase shadow-sm" ><a href="../list/" style="color: white; text-decoration:none">QUAY LẠI</a></button>
              <button type="submit" name="submit" value="1" class="btn btn-primary float-right text-uppercase shadow-sm">CẬP NHẬP</button>
            </div>
          </form> 
        </div>
      </div>
...
<script>
  ...
  function validateForm() {
    ...
    let id_suppliers = document.forms["myForm"]["id_suppliers"].value;
  ...
  <?php 
    ...
    if($submit ==1){
      ...
      $sql_import = "UPDATE `import_details` SET `id_supplier`='$id_supplier', `product_type` = '$details', `quantity` = '$quantity', `price_unit` = '$price_unit', `price_total` = '$price_total', `id_pay_method_import` = '$pay_method', `date_import` = '$date_import', `status` = '1' WHERE `import_details`.`id` = '$id_suppliers';";
      //echo $sql_import;
      //echo exit;
      $run_import = $con->query($sql_import);
      echo '<meta http-equiv="refresh" content="0; url=?id='.$id_suppliers.'&alert=1">';
    }
